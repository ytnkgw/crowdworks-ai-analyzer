...
# Experiment 002 - Technical Spike: Job Detail Page
...
### Objective
一覧ページから取得した案件URLを用いて個別案件ページへアクセスし、HTML構造およびデータ取得方法を調査する。

### Result
- 個別案件ページのHTML取得に成功
- タイトル、案件詳細、報酬、応募期限、掲載日、応募人数、募集人数の取得に成功
- 一覧ページのような案件情報をまとめたJSONデータは存在しないことを確認
- 一部のJSON-LD（`application/ld+json`）は存在するが、AI分析に必要な情報を網羅していない

### Conclusion
一覧ページは埋め込みJSONを解析する方式を採用するが、個別案件ページでは必要な情報がHTML内に分散しているため、各HTMLタグから情報を抽出する方式を採用する。
