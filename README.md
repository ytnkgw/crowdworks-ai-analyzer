...
CrowdWorks AI Analyzer は、CrowdWorksの案件情報を収集し、AIエージェントに渡しやすい構造化データとして出力する Python CLI ツールです。

CrowdWorks上の案件一覧・詳細ページから、案件タイトル、報酬、応募期限、応募人数、クライアント情報、仕事内容などを取得し、jsonファイルとして整理します。出力したデータは、ChatGPTやClaudeなど、ユーザーが普段利用しているAIエージェントに渡して、応募優先度の判断、リスク確認、提案文作成などに活用できます。

OpenAI APIを用いた簡易分析・ランキング機能も備えていますが、本ツールでは特に、AIエージェントが扱いやすい案件データを生成することを重視します。
...
現在は `v0.4.0` として、URLベースの案件収集・JSON出力・AI分析・ランキング出力を整理し、GitHub公開・ポートフォリオ掲載に向けた基本機能を整備しています。
